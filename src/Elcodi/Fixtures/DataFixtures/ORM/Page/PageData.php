...
/**
 * Customer registration email for administrator
 */
		$customerRegistrationEmailAdmin = $pageDirector
			->create()
			->setTitle('Nuovo cliente')
			->setContent('Il cliente {{ customer.fullname }} si è appena registrato. <br> Dati cliente: <br> Nome e cognome: {{ customer.fullname }} <br> indirizzo email: {{ customer.email }}')
			->setName('customer_registration_admin')
			->setType(ElcodiPageTypes::TYPE_EMAIL)
			->setEnabled(true)
			->setPersistent(true);

		$pageDirector->save($customerRegistrationEmailAdmin);
		$this->addReference('email-customer-registration-admin', $customerRegistrationEmailAdmin);

		$entityTranslator->save($customerRegistrationEmailAdmin, [
			'es' => [
				'title' => 'Nuovo cliente',
				'content' => "Il cliente {{ customer.fullname }} si è appena registrato. <br> Dati cliente: <br> Nome e cognome: {{ customer.fullname }} <br> indirizzo email: {{ customer.email }}",
			],
			'it' => [
				'title' => 'Nuovo cliente',
				'content' => "Il cliente {{ customer.fullname }} si è appena registrato. <br> Dati cliente: <br> Nome e cognome: {{ customer.fullname }} <br> indirizzo email: {{ customer.email }}",
