...
class MenuBuilder extends AbstractMenuBuilder implements MenuBuilderInterface
{
    ...
    private $categoryResource = "customerCategory";
    private $permissions = [
        'canRead' => false,
        'canCreate' => false,
        'canUpdate' => false,
    ...
    function __construct(NodeFactory $menuNodeFactory, ContainerInterface $container)
    {
        ...
        $this->permissionsCategory = [
            'canRead' => $this->permissionsRepository->canReadEntity($this->categoryResource, $this->currentUser),
            'canCreate' => $this->permissionsRepository->canCreateEntity($this->categoryResource, $this->currentUser),
            'canUpdate' => $this->permissionsRepository->canUpdateEntity($this->categoryResource, $this->currentUser),
            'canDelete' => $this->permissionsRepository->canDeleteEntity($this->categoryResource, $this->currentUser),
        ];
    }
    ...
    public function build(MenuInterface $menu)
    {
        ...
        if ($this->permissionsCategory['canRead']) {
            $node->addSubnode(
                $this
                    ->menuNodeFactory
                    ->create()
